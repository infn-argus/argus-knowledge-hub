...
def test_schema_icon_replace_and_clear_do_not_leave_orphan_files(token):
    """A type icon isn't attached to any object, so once the type stops
    pointing at it there is no way to reach it again — the row and the file
    have to go with it."""
    suffix = secrets.token_hex(4)
    ...
    first = client.post(
        ...
        headers=auth(token),
    ).json()["icon_attachment_uid"]

    db = SessionLocal()
    first_path = db.get(Attachment, first).storage_path
    db.close()
    ...
    db = SessionLocal()
    assert db.get(Attachment, first) is None
    db.close()
    assert not os.path.exists(first_path)

    cleared = client.delete(f"/v1/schemas/ic-{suffix}/icon", headers=auth(token))
    assert cleared.status_code == 200
    assert cleared.json()["icon_attachment_uid"] is None
    db = SessionLocal()
    assert db.get(Attachment, second) is None
    db.close()
