...
their own, linked to it. The catalogue is seeded into the workspace first if
it is not there. Safe to run again: it updates what the workbook says and
...
Usage (from anywhere, with the backend's virtualenv and DATABASE_URL set):
  python backend/scripts/import_pbs.py <workspace_id> <workbook.xlsx> --facility EUAP [--dry-run]
"""
import os
import sys
...
def main() -> None:
    ...
    rest = [a for i, a in enumerate(argv)
            if not a.startswith("--") and not (i and argv[i - 1] == "--facility")]
    ...
    try:
        if db.get(Workspace, workspace_id) is None:
            ...
            sys.exit(1)
        result = import_pbs(db, workspace_id, book, facility, path.rsplit("/", 1)[-1], dry_run)
    finally:
        db.close()
